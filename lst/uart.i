...
typedef struct
{
 ...
 Struct_DATA AD_Board_Data;

  uint8_t  UART0_Buffer[20];
  uint8_t  UART0_Send_Buffer[20];
  uint8_t  UART1_Buffer[20];
  uint8_t  UART1_Send_Buffer[20];

  uint8_t  AD_BOARD_BL_FLAG;
  uint8_t  AD_BOARD_CHANNAL_FLAG;
  uint8_t  AD_BOARD_BL_STATUS_FLAG;
  uint8_t  BL_MODE_Status;
  uint8_t  BL_LEVEL_Status;
  uint8_t  BL_EN_Status;
  uint8_t  AD_BOARD_CHANNAL_Stasus;
  uint8_t  ADDR_485;
 ...
 
{
	...
	sParam_RS485.u8cAddrEnable = (1<<0); 
	sParam_RS485.u8cAddrValue  = 0x00;	         	 
...
{							 	 
	 ...
	 (*((volatile unsigned long *) (0x40050000UL+0x00000004))) &= ~(1<<0);
	 				   

	 
     if((*((volatile unsigned long *) (0x40050000UL+0x0000001C)))& (1<<0)) 
     {    


		if(DrvGPIO_GetBit(E_PORT0, E_PIN2)==1)
		{
			if(DrvGPIO_GetBit(E_PORT0, E_PIN1)==1)
			{
				ADDR_485 = 0XF5;
...
{							 	 
	...
	(*((volatile unsigned long *) (0x40150000UL+0x00000004))) &= ~(1<<0);


    if((*((volatile unsigned long *) (0x40150000UL+0x0000001C)))& (1<<0)) 
	{    
...
void Uart0_Handle()
{
	...
	if(UART0_RecvFlag==1)
	{
		...
		UART0_Send_Buffer[2] = ADDR_485;	
	 	...
	 	switch (UART0_Buffer[0])
		{
			case 0x01:
				UART0_Send_Buffer[3] = 11;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] =	UART0_Buffer[0];	   
				UART0_Send_Buffer[6] = 'I';					   
				...
				break;
			case 0x02:
				UART0_Send_Buffer[3] = 3;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       

				UART0_Send_Buffer[6] = SENSOR_VALUE;		   
				break;
			case 0x03:
				UART0_Send_Buffer[3] = 3;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       
				UART0_Send_Buffer[6] = TFT_CURR_VALUE;		   
				break;
			case 0x04:
				UART0_Send_Buffer[3] = 3;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       
				UART0_Send_Buffer[6] = BL_CURR_VALUE;		   
				break;
			case 0x05:
				UART0_Send_Buffer[3] = 4;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       
				if(SYS_TEMP_VALUE<40)
				{
					UART0_Send_Buffer[6] = '-';
					UART0_Send_Buffer[7] = 40-SYS_TEMP_VALUE;	
				}
				else
				{
					UART0_Send_Buffer[6] = '+';
					UART0_Send_Buffer[7] = SYS_TEMP_VALUE-40;	
				}
				break;
			case 0x06:
				UART0_Send_Buffer[3] = 4;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       
				if(BACK_TEMP_VALUE<40)
				{
					UART0_Send_Buffer[6] = '-';
					UART0_Send_Buffer[7] = 40-BACK_TEMP_VALUE;	
				}
				else
				{
					UART0_Send_Buffer[6] = '+';
					UART0_Send_Buffer[7] = BACK_TEMP_VALUE-40;	
				}
				break;
			case 0x07:
				UART0_Send_Buffer[3] = 3;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] = UART0_Buffer[0];	       
				UART0_Send_Buffer[6] = IN_VOLTAGE_VALUE;	   
				break;
			...
			case 0x08:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[4] = 'A';					   
					UART0_Send_Buffer[5] = UART0_Buffer[0];	       
					if(DrvGPIO_GetBit(E_PORT4, E_PIN5)==0 && DrvGPIO_GetBit(E_PORT4, E_PIN4)==0)
					{
						UART0_Send_Buffer[6] = 1;				   
					}
					else
					{
						UART0_Send_Buffer[6] = 0;				  
					}	
				}
				else if(UART0_Buffer[2]==1)						  
				{
					if(UART0_Buffer[3]==1)
					{
					...
					{
						DrvGPIO_SetBit(E_PORT4, E_PIN5);
						DrvGPIO_SetBit(E_PORT4, E_PIN4);
					}
					UART0_Send_Buffer[3] = 5;				  	   
					UART0_Send_Buffer[4] = 'A';					   
					UART0_Send_Buffer[5] = UART0_Buffer[0];	       
					if(DrvGPIO_GetBit(E_PORT4, E_PIN5)!=UART0_Buffer[3] && DrvGPIO_GetBit(E_PORT4, E_PIN4)!=UART0_Buffer[3])
					{
						UART0_Send_Buffer[6] = 'O';				   
						UART0_Send_Buffer[7] = 'K';				   
						UART0_Send_Buffer[8] = ' ';				   
					}
					else
					{
						UART0_Send_Buffer[6] = 'E';				  
						UART0_Send_Buffer[7] = 'R';				  
						UART0_Send_Buffer[8] = 'R';				  
					}
				} 
				break;
			case 0x09:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[4] = 'A';					   
					UART0_Send_Buffer[5] = UART0_Buffer[0];	       
					if(DrvGPIO_GetBit(E_PORT0, E_PIN5)==0)
					{
						UART0_Send_Buffer[6] = 1;				   
					}
					else
				...
				else if(UART0_Buffer[2]==1)						  
				{
					...
					{
						DrvGPIO_SetBit(E_PORT0, E_PIN5);
					}
					UART0_Send_Buffer[3] = 5;				  	   
					UART0_Send_Buffer[4] = 'A';					   
					UART0_Send_Buffer[5] = UART0_Buffer[0];	       
					if(DrvGPIO_GetBit(E_PORT0, E_PIN5)!=UART0_Buffer[3])
					{
						UART0_Send_Buffer[6] = 'O';				   
						UART0_Send_Buffer[7] = 'K';				   
						UART0_Send_Buffer[8] = ' ';				   
			...
			case 0x0A:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[6] = AD_Board_Data.AD_Board_BL_EN_Status;	   	   
				}
				else if(UART0_Buffer[2]==1)						  
				{
			...
			case 0x0C:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[6] = AD_Board_Data.AD_Board_BL_Mode_Status;	   	   
					UART0_Send_Buffer[7] = AD_Board_Data.AD_Board_BL_Level_Status;	
				}
				else if(UART0_Buffer[2]==2)						   
				...
				{				
					...
					(uint16_t)(20.0+((275.0-20.0)/100.0)*UART0_Buffer[4]);
			...
			case 0x0d:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[6] = AD_Board_Data.AD_Board_Channel_Status;	   	   
				}
				else if(UART0_Buffer[2]==1)						   
				{				
			...
			case 0x0e:
				if(UART0_Buffer[2]==0)							   
				{
					...
					UART0_Send_Buffer[6] = AD_Board_Data.AD_Board_Monitor_Status;	   	   
				}
				break;
			default:
				UART0_Send_Buffer[3] = 10;				  	   
				UART0_Send_Buffer[4] = 'A';					   
				UART0_Send_Buffer[5] =	UART0_Buffer[0];	   
				UART0_Send_Buffer[6] = 'R';					   
		...
		for(i=1;i<UART0_Send_Buffer[3]+4;i++)			 
		{
			UART0_Send_Buffer[UART0_Send_Buffer[3]+4] += UART0_Send_Buffer[i];
		}
		delay_ms(1);
		DrvUART_Write(UART_PORT0,UART0_Send_Buffer,UART0_Send_Buffer[3]+5);	
...
void Uart1_Handle()
{
	...
	if(UART1_RecvFlag==1)
	{
		...
		if(AD_BOARD_BL_FLAG==1)
		{
			...
			UART1_Send_Buffer[0] = 0xAA;		
			UART1_Send_Buffer[1] = 0x0C;		
			UART1_Send_Buffer[2] = BL_MODE_Status;		
			...
			DrvUART_Write(UART_PORT1,UART1_Send_Buffer,5);	
		}
		if(AD_BOARD_CHANNAL_FLAG==1)
		{
			...
			UART1_Send_Buffer[0] = 0xAA;		
			UART1_Send_Buffer[1] = 0x0D;		
			UART1_Send_Buffer[2] = AD_BOARD_CHANNAL_Stasus;	 
			for(i=1;i<3;i++)			 
			{
				UART1_Send_Buffer[3] += UART1_Send_Buffer[i];
			}
			delay_ms(10);
			DrvUART_Write(UART_PORT1,UART1_Send_Buffer,4);	
		}
		if(AD_BOARD_BL_STATUS_FLAG==1)
		{
			...
			UART1_Send_Buffer[0] = 0xAA;		
			UART1_Send_Buffer[1] = 0x0A;		
			UART1_Send_Buffer[2] = BL_EN_Status;   
			for(i=1;i<3;i++)			 
			{
				UART1_Send_Buffer[3] += UART1_Send_Buffer[i];
			}
