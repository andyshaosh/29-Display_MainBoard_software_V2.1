...
#line 30 "USER\\adc.h"
 ...
 uint32_t TFT_CURR[6];
 uint32_t SENSOR[6];
 uint32_t BACK_TEMP[6];
 uint32_t IN_VOLTAGE[6];
 uint32_t BL_CURR[6];
 uint32_t SYS_TEMP[6];
 uint8_t  TFT_CURR_VALUE;
 uint8_t  SENSOR_VALUE; 
 uint8_t  BACK_TEMP_VALUE;
 uint8_t  IN_VOLTAGE_VALUE;
 uint8_t  BL_CURR_VALUE;
 uint8_t  SYS_TEMP_VALUE;
 uint8_t  ADC_Flag;

 void ADC_init(void);
 uint32_t GetADC(uint8_t ChannelNum);
 void ADC_HANDLE(void);
 uint8_t PowerInputCheck(void);
 

#line 9 "USER\\ADC.c"
#line 1 "USER\\TempCodeDef.H"
...
const float Temperature_Res_Tbl[]=
{
-40,277.2,
-39,263.6,
-38,250.1,
...
void ADC_init(void) 
{
	...
	DrvADC_Open(ADC_SINGLE_END, ADC_SINGLE_CYCLE_OP, 0xF3, INTERNAL_RC22MHZ, 1);
 

 
	(*((volatile unsigned long *) (0x50000000UL+0x0000001C))) = 1;				 


	((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADCHER.PRESEL = 2;	
	...
	for(i=0;i<6;i++)
	{
		TFT_CURR[i]=GetADC(0);
		SENSOR[i]=GetADC(5);
		BACK_TEMP[i]=GetADC(6);
		IN_VOLTAGE[i]=GetADC(4);
		BL_CURR[i]=GetADC(1);
		SYS_TEMP[i]=GetADC(7);
	}
...
void ADC_HANDLE(void)
{
	...
	TFT_CURR[6-1]=GetADC(0);
	SENSOR[6-1]=GetADC(5);
	BACK_TEMP[6-1]=GetADC(6);
	IN_VOLTAGE[6-1]=GetADC(4);
	BL_CURR[6-1]=GetADC(1);
	SYS_TEMP[6-1]=GetADC(7);
	for(i=1;i<6;i++)
	{
		TFT_CURR[0]     += TFT_CURR[i];
		SENSOR[0]       += SENSOR[i]; 
		BACK_TEMP[0]    += BACK_TEMP[i];
		IN_VOLTAGE[0]   += IN_VOLTAGE[i];
		BL_CURR[0]      += BL_CURR[i];
		SYS_TEMP[0]     += SYS_TEMP[i];
	}
	TFT_CURR[0]     /= 6;
	SENSOR[0]       /= 6; 
	BACK_TEMP[0]    /= 6;
	...
	TFT_CURR_VALUE = (2.5-(double)TFT_CURR[0]*4.8/4095)*10/0.94;						  

	if(SENSOR[0] > 4000) SENSOR[0] = 4000;
	if(SENSOR[0] < 100) SENSOR[0] = 100;
	SENSOR_VALUE = (4000-SENSOR[0])*10/(4000-100); 

	IN_VOLTAGE_VALUE = (double)IN_VOLTAGE[0]*4.8/4095*11+0.5;
	BL_CURR_VALUE = (2.5-(double)BL_CURR[0]*4.8/4095)*10/0.3392;						  



	BACK_TEMP_BUF = (double)BACK_TEMP[0]*10/(4095-(double)BACK_TEMP[0]);
	...
	SYS_TEMP_BUF =	(0.724-(double)SYS_TEMP[0]*4.8/4095)*1000/1.85;
